                            ...
                            size_t cnt, loff_t *offt)
{
    ...
    if (atomic_read(&dev->key_info) == 0) //能进read理应是按键唤醒的（因为超时唤醒没让应用调用read，读不到就error）
    {
        return -EAGAIN;
    }
...
unsigned int irq_key_poll(struct file *filp, struct poll_table_struct *wait)
{
    int i;
    unsigned int mask = 0;
    struct devstruct *dev = (struct devstruct *)filp->private_data;

    poll_wait(filp, &dev->r_wait, wait); //wait是在应用陷入内核再到驱动层之前创建的结构体

    for (i = 0; i < KEY_NUM; i++)
    {
        if (atomic_read(&dev->io_irq[i].ok_flag))
        {
            atomic_add(1 << i, &dev->key_info);
        }
    }

    if (atomic_read(&dev->key_info)) //按键按下
    {
    ...
    return mask; //第一次进来发现是0就休眠了，后面无论是超时还是被唤醒，还要再进来一次
                 //这里return的不是return回应用的，而是去内核的，看内核底层代码
                 //回去发现不是0
}
